...
Specifically, it allows any EOA to set its code based on any existing smart contract. To do so, an EOA owner would sign an _authorization_ that could then be submitted by anyone as part of the new transaction type. The code delegation will be valid until replaced by another authorization. The authorization could be given for a single chain, or all chains at once.
